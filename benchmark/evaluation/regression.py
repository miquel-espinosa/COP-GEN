...
class ImageRegressionMetrics:
    def __init__(self):
        ...
        # Per-band accumulators. Keys are band names.
        # For error-based metrics we keep running totals on CPU for stability.
        self._per_band_abs_sum: Dict[str, float] = {}
        self._per_band_sq_sum: Dict[str, float] = {}
        self._per_band_count: Dict[str, int] = {}
        # For SSIM/PSNR per band, keep dedicated metrics per band name
        self._per_band_ssim: Dict[str, StructuralSimilarityIndexMeasure] = {}
        self._per_band_psnr: Dict[str, PeakSignalNoiseRatio] = {}

    @torch.no_grad()
    def update(self, pred: torch.Tensor, target: torch.Tensor, band_names: Dict[int, str] | List[str] | None = None):
        # pred/target: [B, C, H, W]
        p = pred.detach().float()
        ...
        if band_names is None:
            names = [f"band_{i}" for i in range(C)]
        elif isinstance(band_names, dict):
            # mapping channel index -> name
            names = [band_names.get(i, f"band_{i}") for i in range(C)]
        else:
            names = list(band_names)
            assert len(names) == C, "Length of band_names must match number of channels"

        for c in range(C):
            cname = names[c]
            # Error-based metrics
            e = (p[:, c, ...] - t[:, c, ...]).float()
            abs_sum = float(e.abs().sum().item())
            sq_sum = float((e * e).sum().item())
            cnt = int(e.numel())
            self._per_band_abs_sum[cname] = self._per_band_abs_sum.get(cname, 0.0) + abs_sum
            self._per_band_sq_sum[cname] = self._per_band_sq_sum.get(cname, 0.0) + sq_sum
            self._per_band_count[cname] = self._per_band_count.get(cname, 0) + cnt

            # SSIM/PSNR per band use normalized [0,1] based on target per-sample per-band
            t_b = t[:, c:c+1, ...]
            p_b = p[:, c:c+1, ...]
            tb_flat = t_b.view(B, -1)
            tb_min = tb_flat.min(dim=1).values.view(B, 1, 1, 1)
            tb_max = tb_flat.max(dim=1).values.view(B, 1, 1, 1)
            b_denom = (tb_max - tb_min).clamp_min(1e-6)
            t01_b = ((t_b - tb_min) / b_denom).clamp(0.0, 1.0)
            p01_b = ((p_b - tb_min) / b_denom).clamp(0.0, 1.0)
            # Ensure per-band metrics exist and are on correct device
            if cname not in self._per_band_ssim:
                self._per_band_ssim[cname] = StructuralSimilarityIndexMeasure(data_range=(0,1)).to(p.device)
            elif next(self._per_band_ssim[cname].parameters(), torch.tensor(0, device=p.device)).device != p.device:  # type: ignore
                self._per_band_ssim[cname] = self._per_band_ssim[cname].to(p.device)
            if cname not in self._per_band_psnr:
                self._per_band_psnr[cname] = PeakSignalNoiseRatio(data_range=(0,1)).to(p.device)
            elif next(self._per_band_psnr[cname].parameters(), torch.tensor(0, device=p.device)).device != p.device:  # type: ignore
                self._per_band_psnr[cname] = self._per_band_psnr[cname].to(p.device)
            self._per_band_ssim[cname].update(p01_b, t01_b)
            self._per_band_psnr[cname].update(p01_b, t01_b)
    ...
    def summary(self) -> Dict[str, float]:
        ...
        for cname, cnt in self._per_band_count.items():
            if cnt <= 0:
                continue
            mae_b = self._per_band_abs_sum.get(cname, 0.0) / float(cnt)
            rmse_b = (self._per_band_sq_sum.get(cname, 0.0) / float(cnt)) ** 0.5
            ssim_b = float(self._per_band_ssim[cname].compute().item()) if cname in self._per_band_ssim else 0.0
            psnr_b = float(self._per_band_psnr[cname].compute().item()) if cname in self._per_band_psnr else 0.0
            per_band[cname] = {
                'mae': float(mae_b),
                'rmse': float(rmse_b),
                'ssim': ssim_b,
