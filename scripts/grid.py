...
class Grid():
    ...
    def rowcol2latlon_vectorized_needs_testing(self,rows,cols):
        # Vectorized lookup using a cached MultiIndex for speed
        if not hasattr(self, '_points_by_key'):
            # Cache a GeoSeries indexed by (row, col) for O(1) lookups
            self._points_by_key = self.points.set_index(['row','col'])['geometry']
        try:
            keys = pd.MultiIndex.from_arrays([rows, cols], names=['row','col'])
            geoms = self._points_by_key.loc[keys]
        except KeyError:
            # Fallback: merge-based lookup to surface which keys are missing
            lookup = pd.DataFrame({'row': rows, 'col': cols})
            merged = lookup.merge(self.points[['row','col','geometry']], on=['row','col'], how='left')
            if merged['geometry'].isna().any():
                missing = merged[merged['geometry'].isna()][['row','col']].drop_duplicates().values.tolist()
                raise KeyError(f"Missing row/col combinations in grid: {missing}")
            geoms = gpd.GeoSeries(merged['geometry'])
        lats = geoms.y.tolist()
        lons = geoms.x.tolist()
        return lats, lons
