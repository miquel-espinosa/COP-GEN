...
from visualisations.thumbnails_vis import get_hillshade_torch_differentiable
...
class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, *, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 ...
                 disc_loss="hinge", hillshade_weight=0.0):
        ...
        self.hillshade_weight = hillshade_weight
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
        ...
        hillshade_loss = torch.tensor(0.0, device=inputs.device)
        if self.hillshade_weight > 0:
            if data_cfg is None:
                raise ValueError("data_cfg is required for hillshade loss")
            assert data_cfg['satellite_type'] == "DEM"
            
            inputs_norm = post_process_data(inputs, data_cfg['satellite_type'], data_cfg)
            recon_norm = post_process_data(reconstructions, data_cfg['satellite_type'], data_cfg)
            
            input_hillshade = get_hillshade_torch_differentiable(inputs_norm, normalize_per_image=False)
            recon_hillshade = get_hillshade_torch_differentiable(recon_norm, normalize_per_image=False)
            
            # DEBUG PLOTTING ORIGINAL VS DIFFERENTIABLE=================================================================================
            # from ddm.pre_post_process_data import post_process_data
            # from visualisations.thumbnails_vis import get_hillshade_torch
            # import matplotlib.pyplot as plt
            # import torchvision as tv
            # inputs_norm_original = post_process_data(inputs, "DEM", was_normalized_to_neg_one_to_one=True, min_db=-63.531815, max_db=38.483150, min_positive=4.43423601836912e-07)
            # recon_norm_original = post_process_data(reconstructions, "DEM", was_normalized_to_neg_one_to_one=True, min_db=-63.531815, max_db=38.483150, min_positive=4.43423601836912e-07)
            
            # inputs_original_hillshade = get_hillshade_torch(inputs_original.detach())
            # recon_original_hillshade = get_hillshade_torch(recon_original.detach())
            
            # # Plot the two images side by side
            # comparison = torch.cat([inputs_original_hillshade, recon_original_hillshade], dim=0)
            # tv.utils.save_image(comparison, "original_DEM_hillshade_debug.png", nrow=1)
            # print("original_DEM_hillshade_debug.png saved")
            
            # # Plot the two images side by side
            # comparison = torch.cat([input_hillshade, recon_hillshade], dim=0)
            # tv.utils.save_image(comparison, "differentiable_DEM_hillshade_debug.png", nrow=1)
            # print("differentiable_DEM_hillshade_debug.png saved")
            # exit()
            # END DEBUG==================================================================================================================
            
            hillshade_loss = torch.abs(input_hillshade - recon_hillshade) + \
                             F.mse_loss(input_hillshade, recon_hillshade, reduction="none")
            hillshade_loss = torch.mean(hillshade_loss)
        ...
        if optimizer_idx == 0:
            ...
            loss = weighted_nll_loss + self.kl_weight * kl_loss + d_weight * disc_factor * g_loss + self.hillshade_weight * hillshade_loss
            ...
            log = {"{}/total_loss".format(split): loss.clone().detach().mean(), "{}/logvar".format(split): self.logvar.detach(),
                   ...
                   "{}/hillshade_loss".format(split): hillshade_loss.detach().mean(),
                   }
            return loss, log
