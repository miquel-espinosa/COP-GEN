...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self):
        ...
        # No magic number scaling? https://github.com/huggingface/diffusers/issues/437
        # x = x * 0.18215
        return x
...
class AutoencoderKL(nn.Module):
    def __init__(self,
        ...
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys, model_isloaded=model_isloaded)

    def init_from_ckpt(self, path, ignore_keys=list(), use_ema=True, model_isloaded=False):
        sd = torch.load(path, map_location="cpu")
        sd_keys = sd.keys()
        if model_isloaded:
            sd = sd
        elif 'ema' in list(sd.keys()) and use_ema:
            sd = sd['ema']
            new_sd = {}
            for k in sd.keys():
    ...
    def sample_moments(self, moments):
    ...
    def training_step(self, inputs, optimizer_idx, global_step, data_cfg):
        ...
        if optimizer_idx == 0:
            ...
            aeloss, log_dict_ae = self.loss(inputs, reconstructions, posterior, optimizer_idx, global_step,
                                            last_layer=self.get_last_layer(), split="train", data_cfg=data_cfg)
    ...
    def validation_step(self, inputs, global_step, data_cfg):
        # inputs = self.get_input(batch, self.image_key)
        reconstructions, posterior = self(inputs)
        aeloss, log_dict_ae = self.loss(inputs, reconstructions, posterior, 0, global_step,
                                        last_layer=self.get_last_layer(), split="val", data_cfg=data_cfg)
