...
def modality_configs():
    ...
    all_modality_configs = {
        "DEM_thumbnail": d(
            ...
            latent_input_resolution=132, # latent input size (4, 32, 32)
            latent_crop_resolution=32,   # latent crop  size (4, 32, 32)
            img_bands=3,                 # number of bands   (3)
            img_input_resolution=256,    # input pixel space (3, 32, 32)
            scale_factor='sd',
        ),
        "S1RTC_thumbnail": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
...
def get_config():
    ...
    config.train = d(
        ...
        batch_size=8,  # Change to 512
        mode="uncond",
        log_interval=100, # Change to 100
        eval_interval=5, # Change to 1500
        save_interval=5000,
        multi_modal=True,
    )
    config.optimizer = d(
    ...
    config.dataset = d(
        ...
        path="data/majorTOM/small_world/copgenbeta_test/train",
        cfg=False,
        p_uncond=0.1, # 0.15
        random_flip=True,
