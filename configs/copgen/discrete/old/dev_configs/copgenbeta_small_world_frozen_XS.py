...
def modality_configs():
    """
        ...
        NOTE: The order of the modalities matters. Use alphabetical order.
    ...
    all_modality_configs = {
        ...
        #     latent_input_resolution=8,   # latent input size (4, 8, 8)
        #     latent_crop_resolution=8,    # latent crop  size (4, 8, 8)
        #     img_bands=3,                 # number of bands   (3)
        #     img_input_resolution=64,     # input pixel space (3, 64, 64)
        #     scale_factor='sd',
        # ),
        "S1RTC_vh_vv_vv+vh": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
            latent_channels=4,           # latent z_channels (4, *, *)
            latent_input_resolution=24,  # latent input size (4, 24, 24)
            latent_crop_resolution=24,   # latent crop  size (4, 24, 24)
            img_bands=3,                 # number of bands   (3)
            img_input_resolution=192,    # input pixel space (3, 192, 192)
            scale_factor='sd',
        ),
        "S2L2A_B01_B01+B09_B09": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
            latent_channels=4,           # latent z_channels (4, *, *)
            latent_input_resolution=4,   # latent input size (4, 4, 4)
            latent_crop_resolution=4,    # latent crop  size (4, 4, 4)
            img_bands=3,                 # number of bands   (3)
            img_input_resolution=32,     # input pixel space (3, 32, 32)
            scale_factor='sd',
        ),
        "S2L2A_B02_B03_B04": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
        ...
        "S2L2A_B05_B06_B07": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
            latent_channels=4,           # latent z_channels (4, *, *)
            latent_input_resolution=12,  # latent input size (4, 12, 12)
            latent_crop_resolution=12,   # latent crop  size (4, 12, 12)
            img_bands=3,                 # number of bands   (3)
            img_input_resolution=96,     # input pixel space (3, 96, 96)
            scale_factor='sd',
        ),
        "S2L2A_B08_B08_B08": d(
            autoencoder_path="assets/stable-diffusion/autoencoder_kl_ema.pth",
...
def get_config():
    ...
    config.dataset = d(
        ...
        path="data/majorTOM/small_world/latents_frozen_XS_lmdb/train",
        cfg=False,
        p_uncond=0.1, # 0.15
        random_flip=True,
