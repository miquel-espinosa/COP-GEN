...
class FrozenAutoencoderKL(nn.Module):
    def __init__(self, ddconfig, embed_dim, pretrained_path, scale_factor=None):
        super().__init__()
        ...
        if scale_factor is None:
            raise ValueError("scale_factor must be provided. \ Run `scripts/compute_scaling_factor.py` to get the scaling factor for the given autoencoder.")
        elif scale_factor == 'sd':
    ...
    def sample(self, moments):
        ...
        # Magic number. # See https://github.com/huggingface/diffusers/issues/437
...
def get_model(pretrained_path, scale_factor=0.18215, ddconfig=None):
    if ddconfig is None:
        ddconfig = dict(
            double_z=True,
            z_channels=4,
    ...
    return FrozenAutoencoderKL(ddconfig, ddconfig["z_channels"], pretrained_path, scale_factor)
